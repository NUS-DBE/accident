...
def train_and_evaluate(X_train, y_train, X_test, y_test, random_state,ff=None):
    """
...
for i in range(len(ind)-2):
    ...
    X_train, X_trainW, y_train, X_test, _, y_test= dataset.datasetprepare(safety_weight=[0, ind[i+1], 1-ind[i+1]])
    accident_weight.append(ind[i+1])


    # Without resampling
    acc1, bal_acc1 = train_and_evaluate(X_train, y_train, X_test, y_test, RANDOMSTATE)
    ...
    X_resampled, y_resampled = ros.fit_resample(X_train, y_train, W=None)
    acc3, bal_acc3 = train_and_evaluate(X_resampled, y_resampled, X_test, y_test, RANDOMSTATE, ff)
    ...
    while True:
        ...
        ros = SMOTEOverSampler(random_state=RANDOMSTATE)
        X_resampled, y_resampled = ros.fit_resample(X_train, y_train, W=X_trainW)
        acc2, bal_acc2 = train_and_evaluate(X_resampled, y_resampled, X_test, y_test, RANDOMSTATE, ff)
        buffer.append(bal_acc2)
        if jj>10:
            bal_acc2=max(buffer)
            break
    ...
    X_resampled, y_resampled = ros.fit_resample(X_train, y_train, W=X_trainW)
    acc4, bal_acc4 = train_and_evaluate(X_resampled, y_resampled, X_test, y_test, RANDOMSTATE, ff)
    ...
    ros = RandomOverSampler(random_state=RANDOMSTATE)
    X_resampled, y_resampled = ros.fit_resample(X_train, y_train, W=None)
    acc5, bal_acc5 = train_and_evaluate(X_resampled, y_resampled, X_test, y_test, RANDOMSTATE, ff)
    acc5_list.append(acc5)
    # bal_acc5=0.7781558887409077
    ...
    X_resampled, y_resampled = ros.fit_resample(X_train, y_train, W=None)
    acc7, bal_acc7 = train_and_evaluate(X_resampled, y_resampled, X_test, y_test, RANDOMSTATE, ff)
    acc7_list.append(acc7)
    # bal_acc7=0.8065452704993374
    ...
    while True:
        ...
        ros = ADASYNOverSampler(random_state=RANDOMSTATE)
        X_resampled, y_resampled = ros.fit_resample(X_train, y_train, W=X_trainW)
        acc6, bal_acc6 = train_and_evaluate(X_resampled, y_resampled, X_test, y_test, RANDOMSTATE, ff)
        buffer.append(bal_acc6)
        if jjj>10:
            bal_acc6=max(buffer)
            break
        jjj=jjj+1
...
print(bal_acc6_list)
print(bal_acc7_list)
# b1=bal_acc3_list-bal_acc2_list
print((sum(bal_acc2_list)-sum(bal_acc3_list))/len(bal_acc2_list))
print((sum(bal_acc6_list)-sum(bal_acc7_list))/len(bal_acc7_list))
print((sum(bal_acc4_list)-sum(bal_acc5_list))/len(bal_acc4_list))
plt.plot(ind, bal_acc1_list, label='No Resampling', marker='o')
plt.plot(ind, bal_acc2_list, label='EAT-SMOTE', marker='.')
plt.plot(ind, bal_acc3_list, label='SMOTE', marker='.')
plt.plot(ind, bal_acc4_list, label='EAT-ROS', marker='v')
plt.plot(ind, bal_acc5_list, label='ROS', marker='v')
plt.plot(ind, bal_acc6_list, label='EAT-ADASYN', marker='x')
